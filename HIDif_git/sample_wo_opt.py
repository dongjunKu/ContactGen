...
dataloader_test = DataLoader(dataset_test, 1, shuffle=False)

# models
# basenet = BaseNet(channels=3+6+32, num_layers=num_layers).to(device) # transl + rot (encoded) + poz
# basenet = BaseNetNoPartner().to(device)
# basenet = BaseNetX4(ch_hidden=512).to(device) # ci3d_X4_512_subtlerefine
...
for step, data in enumerate(dataloader_test):
    ...
    human_feature_pred, human_feature_steps = basecddpm.sample(partner_feature, interaction_type, num_samples, cfg_scale1, cfg_scale2)
    ...
    save_pkl({'transl': human_transl_gt,
                ...
                'right_hand_pose': human_rh_pose_gt}, outpath=osp.join(out_dir, f'step{step:04d}_{interaction_type_string}_human_gt'))
    save_pkl({'transl': human_transl_steps,
                'global_orient': human_rot3_steps,
                'pose': human_pose_steps,
                'left_hand_pose': human_lh_pose_steps,
                'right_hand_pose': human_rh_pose_steps}, outpath=osp.join(out_dir, f'step{step:04d}_{interaction_type_string}_human_pred'))
