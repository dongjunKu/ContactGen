...
class DensePoseMethods:
    def __init__(self):
        ...
        ALP_UV = loadmat(os.path.join(os.path.dirname(
            __file__), 'UV_data/UV_Processed.mat'))
        ...
        UV_symmetry_filename = os.path.join(os.path.dirname(
            __file__), 'UV_data/UV_symmetry_transforms.mat')
    ...
    def get_symmetric_densepose(self, I, U, V, x, y, Mask):
        ...
        for i in (range(24)):
            if i+1 in I:
                ...
                ###
                V_sym[jj] = self.UV_symmetry_transformations['V_transforms'][0,
                                                                             i][V_loc, U_loc]
                U_sym[jj] = self.UV_symmetry_transformations['U_transforms'][0,
                                                                             i][V_loc, U_loc]
        ##
    ...
    def IUV2FBC(self, I_point, U_point, V_point):
        ...
        #
        D1 = scipy.spatial.distance.cdist(np.array([U_point, V_point])[
                                          np.newaxis, :], P_0[:, 0:2]).squeeze()
        D2 = scipy.spatial.distance.cdist(np.array([U_point, V_point])[
                                          np.newaxis, :], P_1[:, 0:2]).squeeze()
        D3 = scipy.spatial.distance.cdist(np.array([U_point, V_point])[
                                          np.newaxis, :], P_2[:, 0:2]).squeeze()
        #
        minD1 = D1.min()
        minD2 = D2.min()
        minD3 = D3.min()
    ...
    # GU
    def FBC2PointID(self, FaceIndex, bc1, bc2, bc3):

        Vert_indices = self.All_vertices[self.FacesDensePose[FaceIndex]] - 1

        return Vert_indices[np.argmax([bc1, bc2, bc3])]

    # GU
    def PointID2IUVTable(self):
        # smpl vertex idx -> densepose vertex idx -> densepose iuv
        densepose_vidx_list = np.zeros(6890, dtype=np.int)
        for i, v in enumerate(self.All_vertices):
            densepose_vidx_list[v - 1] = i

        I_list = []
        U_list = self.U_norm[densepose_vidx_list]
        V_list = self.V_norm[densepose_vidx_list]
        for vidx in densepose_vidx_list:
            faceIndices = np.where(self.FacesDensePose == vidx)[0]
            faceIndex = faceIndices[0]  # FIXME 그냥 첫 번째 표면 선택
            faceLabel = self.FaceIndices[faceIndex]
            I_list.append(faceLabel)

        ret = [(i, u, v) for i, u, v in zip(I_list, U_list, V_list)]

        return ret

dputils = DensePoseMethods()