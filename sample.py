...
from optimizer import callback_factory
from params import ParamsSampleDiffusion, ParamsSampleGuideNet
from utils import save_pkl, smplx_utils

# dataset
dataset_test = CI3D(ParamsSampleDiffusion.dataset_dir, mode='test')
dataloader_test = DataLoader(dataset_test, batch_size=ParamsSampleDiffusion.batch_size, shuffle=False)
...
baseguidenet = BaseGuideNet(num_classes=ParamsSampleGuideNet.num_label).to(ParamsSampleGuideNet.device)
...
for step, data in enumerate(dataloader_test):
    ...
    callback = callback_factory(None, baseguidenet)

    human_feature_pred, human_feature_steps, outdicts = basecddpm.sample(partner_feature, interaction_type, ParamsSampleDiffusion.num_samples, ParamsSampleDiffusion.cfg_scale1, ParamsSampleDiffusion.cfg_scale2, callback)
    ...
    save_pkl({'transl': human_transl_steps,
                ...
                'outdicts': outdicts}, outpath=osp.join(ParamsSampleDiffusion.out_dir, f'step{step:04d}_{interaction_type_string}_human_pred'))
